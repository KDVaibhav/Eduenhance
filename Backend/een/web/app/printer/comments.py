...
from een.db import comments, users, proposals
...
def print_comments(requester, criteria):
    ...
    path = os.path.abspath("een/web/app/printer/")

    p = "./een/web/app/printer/"
    ...
    html = template.render(requester=requester, rows=rows, proposal=proposal, path=path, now=now)
